...
def chart_daily_patterns(cea_data, selected_buildings, output_mode):
    ...
    """
    df = None
    source_is_native_building = False
    if selected_buildings:
        radiation_frames = []
        for building in selected_buildings:
            radiation_df = cea_data["files"].get(f"{building}_radiation.csv")
        ...
        if radiation_frames:
            ...
            for extra in radiation_frames[1:]:
                if len(extra) == len(df):
                    extra_surface_map = {
                        "Roof":       next((c for c in extra.columns if "roof" in c.lower() and "window" not in c.lower()), None),
                        "South wall": next((c for c in extra.columns if "south" in c.lower() and "window" not in c.lower()), None),
                        "East wall":  next((c for c in extra.columns if "east" in c.lower() and "window" not in c.lower()), None),
                        "West wall":  next((c for c in extra.columns if "west" in c.lower() and "window" not in c.lower()), None),
                        "North wall": next((c for c in extra.columns if "north" in c.lower() and "window" not in c.lower()), None),
                    }
                    for col in extra_surface_map.values():
                        if col:
                            if col not in df.columns:
                                df[col] = 0
                            df[col] = _num(df[col]) + _num(extra[col])
            source_is_native_building = True
    ...
    scale_factors = {surface: 1.0 for surface in found}
    scaled_to_building = False
    if selected_buildings and not source_is_native_building:
        annual_df = cea_data["files"].get("solar_irradiation_annually_buildings.csv")
        name_col = _find_col(annual_df, "name", "Name", "building") if annual_df is not None else None
        if annual_df is not None and name_col:
            annual_use = annual_df[annual_df[name_col].isin(selected_buildings)].copy()
            annual_map = {
                "Roof":       next((c for c in annual_use.columns if "roof" in c.lower() and "window" not in c.lower()), None),
                "South wall": next((c for c in annual_use.columns if "south" in c.lower() and "window" not in c.lower()), None),
                "East wall":  next((c for c in annual_use.columns if "east" in c.lower() and "window" not in c.lower()), None),
                "West wall":  next((c for c in annual_use.columns if "west" in c.lower() and "window" not in c.lower()), None),
                "North wall": next((c for c in annual_use.columns if "north" in c.lower() and "window" not in c.lower()), None),
            }
            for surface, col in found.items():
                annual_col = annual_map.get(surface)
                scenario_total = float(_num(df[col]).sum())
                selected_total = float(_num(annual_use[annual_col]).sum()) if annual_col else 0.0
                if scenario_total > 0 and selected_total > 0:
                    scale_factors[surface] = selected_total / scenario_total
                    scaled_to_building = True
    ...
    for _, row in df.iterrows():
        for surface, col in found.items():
            rows.append({
                "Hour": row["_hour"],
                "Surface": surface,
                "Irradiation (kWh)": float(pd.to_numeric(pd.Series([row[col]]), errors="coerce").fillna(0).iloc[0]) * scale_factors.get(surface, 1.0)
            })
    df_long = pd.DataFrame(rows)
    df_long = (
    ...
    if source_is_native_building:
        title = "Building-level average daily solar radiation profile by surface"
    else:
        title = "Scaled average daily irradiation profile by surface" if scaled_to_building else "Average daily irradiation profile by surface"
