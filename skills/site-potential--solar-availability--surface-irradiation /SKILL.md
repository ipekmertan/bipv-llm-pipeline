...
This skill reads CEA's solar irradiation output files and compares radiation received across all building surfaces (roof, and facade walls by orientation: N/S/E/W). It filters out windows (not installable) and ranks opaque surfaces by annual irradiation received.
...
- Comparison against the 800/1000 kWh/m²/year benchmark
- Clear label for each surface: "above threshold ✓" or "below threshold ✗"
...
- Colour: green if above threshold, amber if borderline, red if below
...
- One row per surface with label and value
...
- **Total kWh vs kWh/m²:** CEA gives total kWh, not intensity. A large north wall can show higher total kWh than a small south wall even though it is less efficient per m². Always note this distinction in the output.
